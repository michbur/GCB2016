...
ggplot(si_dat, aes(x=si, y=AUC_mean)) + 
  ...
  scale_fill_continuous("Density", low = "#ffd4d4", high = "#8b0000") +
  xlab("Similarity index") +
  ylab("AUC") +
  my_theme +
  geom_point(si_dat, # data = filter(si_dat, et != "Encoding"),
             aes(x = si, y = AUC_mean, color = et, shape = et), size = 4) +
  guides(color = guide_legend(nrow = 2), shape = guide_legend(nrow = 1)) +
  scale_shape_manual("", values = c(16, 16, 1, 15, 15), drop = FALSE) +
  scale_color_manual("", values = c("grey", "red", "grey", "green", "blue"), drop = FALSE) +
