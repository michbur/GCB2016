...
clusterization, which was performed 
...
<<echo = FALSE,message=FALSE,fig.align='center',fig.width=14,fig.height=5,warning=FALSE>>=
si_dat <- read.csv2("si_dat.csv")

levels(si_dat[["et"]]) <- c("Best-performing encoding", 
                            "Encoding", "Full alphabet", 
                            "Standard encoding", 
                            "Standard encoding")
...
@
Similarity index~\citep{stephenson_unearthing_2013} measures the similarity between two encodings (1 - identical, 0, totally dissimilar). The more similar is given encoding to the best-performing encoding, the higher AUC has classifier that employs it. Such relationship indicates that the best-performing encoding was not found by chance, but represents properties required for the proper prediction of amyloids. 
...
in classification of the shortest sequences.
