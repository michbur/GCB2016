...
The reduced amino acid alphabets with the lowest sum of ranks from all sequence length categories (assuming the rank 1 for the best AUC, rank 2 for the second best AUC and so on) was selected as the best one.
  \end{frame}
